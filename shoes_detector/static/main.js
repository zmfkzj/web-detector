...
startBtn.onclick = async () => {
  ...
  doneFiles = 0;
  prog.value = 0; prog.max = totalFiles || 1;
...
async function processAllFiles() {
  ...
  let i = 0;
  const runners = Array.from({ length: Math.min(CONCURRENCY, queue.length) }, () => workerRunner());
  ...
  async function workerRunner() {
    while (i < queue.length) {
      ...
      try {
        ...
        worker.postMessage({ type: 'process', imageBytes: imgBytes, baseName: base }, [imgBytes]);
      } catch (err) {
